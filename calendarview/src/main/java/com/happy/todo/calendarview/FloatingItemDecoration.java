...
public class FloatingItemDecoration extends RecyclerView.ItemDecoration {  
    ...
    public FloatingItemDecoration(Context context) {  
        ...
        mDivider = a.getDrawable(Color.TRANSPARENT);
        a.recycle();  
    ...
    private void init(Context mContext) {  
        ...
        mTextPaint.setColor(Color.parseColor("#3F4663"));
        ...
        shadowPaint.setColor(Color.parseColor("#10151515"));
        shadowPaint.setAntiAlias(true);
        shadowPaint.setStrokeWidth(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, mContext.getResources().getDisplayMetrics()));
        shadowPaint.setShadowLayer(3,0,0, Color.parseColor("#10151515"));
    }  
...
//                Log.e(CurrencyTag, "onDrawOver: "+child.getTop()+"$"+firstVisiblePos );
        ...
        float x = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 148, mContext.getResources().getDisplayMetrics());
        float y = bottom - (mTitleHeight - mTextHeight) / 2 - mTextBaselineOffset;//计算文字baseLine  
        c.drawText(title, x, y, mTextPaint);
        c.drawLine(left,bottom,right,bottom,shadowPaint);
        if (flag) {  
            //还原画布为初始状态  
            c.restore();  
        }  
