...
public class CalendarPickerView extends RecyclerView {
    ...
    private static int monthsTitle[] = {
...
//        setCacheColorHint(bg);
    ...
    private void validateAndUpdate() {
        if (getAdapter() == null) {
            setAdapter(adapter);
            FloatingItemDecoration itemDecoration = new FloatingItemDecoration(mContext);
            Map<Integer, String> keys = new HashMap<>();
            int index = 0;
            for (MonthDescriptor monthDescriptor : months){
                keys.put(index, getResources().getString(R.string.year_title,monthDescriptor.getYear()) + mContext.getString(monthsTitle[monthDescriptor.getMonth()]));
                index++;
            }
            itemDecoration.setKeys(keys);
            itemDecoration.setmTitleHeight((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40, mContext.getResources().getDisplayMetrics()));
