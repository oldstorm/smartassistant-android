...
public class OverlayView extends View {
    ...
    public boolean isShowCropCorner() {
        return mShowCropCorner;
    }

    public void setShowCropCorner(boolean showCropCorner) {
        this.mShowCropCorner = showCropCorner;
    }
    ...
    public void setTargetAspectRatio(final float targetAspectRatio) {
        ...
        Log.e("mTargetAspectRatio", "mTargetAspectRatio的值：" + mTargetAspectRatio);
        if (mThisWidth > 0) {
            setupCropBounds();
            postInvalidate();
        } else {
...
//            mCropViewRect.set(getPaddingLeft() + halfDiff, getPaddingTop()+TOP_BOTTOM_MARGIN,
//                    getPaddingLeft() + width + halfDiff, getPaddingTop()-TOP_BOTTOM_MARGIN + mThisHeight);
//        } else {
//            int halfDiff = (mThisHeight - height) / 2;
//            mCropViewRect.set(getPaddingLeft(), getPaddingTop() + TOP_BOTTOM_MARGIN + halfDiff,
//                    getPaddingLeft() + mThisWidth, getPaddingTop() -TOP_BOTTOM_MARGIN + height + halfDiff);
//        }


        int size = (int) (Math.min(mThisWidth, mThisHeight));
        float leftOffsets = (mThisWidth-size)  / 2;
        float topOffsets = (mThisHeight-size) / 2;
        mCropViewRect.set(getPaddingLeft(), (int) topOffsets, getPaddingLeft() + size, (int) topOffsets + size);
        if (mCallback != null) {
...
//        }
    ...
    protected void drawCropGrid(@NonNull Canvas canvas) {
        ...
        // 显示四个角
        if (mShowCropCorner || mFreestyleCropMode != FREESTYLE_CROP_MODE_DISABLE) {
            canvas.save();
